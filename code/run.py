...
DATA_PROCESS_CLASS = {
    ...
    'bert-vit-inter-matrix-re': (MREProcessor, MREDataset),
    'bert-vit-inter-matrix-re-BI': (MREProcessor, MREDataset),
    'bert-vit-inter-matrix-overlap-re-BI': (MREOverlapProcessor, MREOverlapDataset),
    'bert-vit-inter-matrix-re-BI-test':(MREOverlapProcessor_test, MREOverlapDataset_test)
}

MODEL_CLASS = {
    'bert-vit-inter-re': BertVitInterReModel,
    'bert-vit-inter-matrix-re': BertVitInterMatrixReModel,
    'bert-vit-inter-matrix-re-BI': BertVitInterMatrixReModelBI,
    'bert-vit-inter-matrix-overlap-re-BI': BertVitInterMatrixReModelBI,
    'bert-vit-inter-matrix-re-BI-test': BertVitInterMatrixReModelBI, 
...
DATA_PATH = {
    'mnre': {'train': 'data/mnre/txt/ours_train.txt',
             ...
             'test': 'data/mnre/txt/ours_test_s.txt',
             'train_auximgs': 'data/mnre/txt/mre_train_dict.pth',  # {data_id : object_crop_img_path}
             'dev_auximgs': 'data/mnre/txt/mre_dev_dict.pth',
             'test_auximgs': 'data/mnre/txt/mre_test_dict_s.pth',
             'train_img2crop': 'data/mnre/img_detect/train/train_img2crop.pth',
             'dev_img2crop': 'data/mnre/img_detect/val/val_img2crop.pth',
             'test_img2crop': 'data/mnre/img_detect/test/test_img2crop_s.pth'},
    'jmere': {'train': 'data/jmere/txt/ours_train.json',
             'dev': 'data/jmere/txt/ours_val.json',
             'test': 'data/jmere/txt/ours_test.json',
            'train_auximgs': 'data/jmere/txt/mre_train_dict.pth',  # {data_id : object_crop_img_path}
             'dev_auximgs': 'data/jmere/txt/mre_dev_dict.pth',
             'test_auximgs': 'data/jmere/txt/mre_test_dict.pth',
             'train_img2crop': 'data/jmere/img_detect/train/train_img2crop.pth',
             'dev_img2crop': 'data/jmere/img_detect/val/val_img2crop.pth',
             'test_img2crop': 'data/jmere/img_detect/test/test_img2crop.pth'},
}
...
IMG_PATH = {
    ...
    'jmere':{'train': 'data/jmere/img_org/train/',
             'dev': 'data/jmere/img_org/val/',
             'test': 'data/jmere/img_org/test'},
}

AUX_PATH = {
    'mnre': {'train': 'data/mnre/img_vg/train/crops',
            'dev': 'data/mnre/img_vg/val/crops',
            'test': 'data/mnre/img_vg/test/crops'},
    'jmere':{'train': 'data/jmere/img_vg/train/crops',
            'dev': 'data/jmere/img_vg/val/crops',
            'test': 'data/jmere/img_vg/test/crops'},
}
...
def parse_argument():
    ...
    parser.add_argument('--model_name', default='bert-vit-inter-matrix-overlap-re-BI', type=str, help="The name of bert.")
    ...
    parser.add_argument('--lr', default=1e-5, type=float, help="learning rate")  #original 1e-5  overlapp 5e-5
    parser.add_argument('--classifier_lr', default=3e-5, type=float, help="classifier learning rate")  #original 1e-5  overlapp 5e-5
    ...
    parser.add_argument('--load_path', default="test_overlap_ori_matrix_mnre_2024_12_30_21_56_22_test", type=str, help="Load model from load_path")
    ...
    parser.add_argument('--do_train',  default=True) #action='store_true')# action='store_true')  # #default=True) #action='store_true') 
    parser.add_argument('--do_test', default=True)#action='store_true') #default=True)#
    parser.add_argument('--do_predict', default=True)#action='store_true')
    parser.add_argument('--prompt_len', default=4, type=int)
    parser.add_argument('--max_seq', default=60, type=int)
    ...
    parser.add_argument('--overlap_del_ratio', default=0.3, type=float, help="overlapping dataset del no rel instances ratio")
...
def init_and_train_bert_vit_re(args, logger):
    ...
    if args.do_train:
        ...
        trainer = BertVitReTrainer(train_data=train_dataloader, dev_data=dev_dataloader, test_data=test_dataloader,
                                   re_dict=re_dict, model=model, args=args, logger=logger, writer=None,process=processor)
        trainer.train()
...
if __name__ == "__main__":
    ...
    try:
        TRAINER = {
            ...
            'bert-vit-inter-matrix-re': init_and_train_bert_vit_re,
            'bert-vit-inter-matrix-re-BI': init_and_train_bert_vit_re,
            'bert-vit-inter-matrix-overlap-re-BI': init_and_train_bert_vit_re,
            'bert-vit-inter-matrix-re-BI-test': init_and_train_bert_vit_re
        }
