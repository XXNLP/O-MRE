...
class CLIPVisionEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.aux_position_embedding = nn.Embedding(48, self.embed_dim)  #original is 48
        self.register_buffer("aux_position_ids", torch.arange(48).expand((1, -1))) #original is 48
...
class VisualLayerWiseMoE(nn.Module):
    ...
    def forward(self,
        ...
        expert_router_input = torch.cat((text_hidden_states,
                                         torch.cat(layer_wise_visual_hidden_states, dim=-1),), dim=-1)  # text+img
        ...
        #layer_wise_visual_hidden_states = torch.stack((layer_wise_visual_hidden_states), dim=2)
...
class MutualInforLayerWiseAgg(nn.Module):
    def __init__(self, config, args, aggred_layer_num=1):
        super().__init__()
        self.args = args
        self.t2v_cross_modal_att_layer = nn.ModuleList(
            [Vision2TextAttOnlyLayer(config, args, layer=i) for i in range(aggred_layer_num)])
        self.v2t_cross_modal_att_layer = nn.ModuleList(
            [Vision2TextAttOnlyLayer(config, args, layer=i) for i in range(aggred_layer_num)])
        self.moe_layer_wise_agg = VisualLayerWiseMoE(config, aggred_layer_num)
        self.aggred_layer_num = aggred_layer_num

        self.commonExperts = Vision2TextAttOnlyLayer(config, args, layer=0) 
        #self.commonExperts = nn.Parameter(torch.Tensor(self.n_experts, config.hidden_size, config.hidden_size),requires_grad=True)
    ...
    def forward(
        ...
        t2v_layer_wise_visual_hidden_states = ()
        v2t_layer_wise_text_hidden_states = ()

        t2v_cross_modal_attentions = () if output_attentions else None
        v2t_cross_modal_attentions = () if output_attentions else None

        t2v_text_hidden_states = all_text_hidden_states[-1]
        v2t_vision_hidden_states = all_vision_hidden_states[-1]
        ...
        for i in range(self.aggred_layer_num):
            detached_vision_hidden_states += (all_vision_hidden_states[i].detach(),)
        for i in range(self.aggred_layer_num):
            t2v_attention_outputs = self.t2v_cross_modal_att_layer[i](t2v_text_hidden_states,
                                                              detached_vision_hidden_states,
                                                              output_attentions)
            
            v2t_attention_outputs = self.v2t_cross_modal_att_layer[i](v2t_vision_hidden_states,
                                                              detached_text_hidden_states,
                                                              output_attentions)
            if output_attentions:
                t2v_cross_modal_attentions += (t2v_attention_outputs[1],)
                v2t_cross_modal_attentions += (v2t_attention_outputs[1],)
            t2v_layer_wise_visual_hidden_states += (t2v_attention_outputs[0],)
            v2t_layer_wise_text_hidden_states += (v2t_attention_outputs[0],)

        t2v_cross_modal_hidden_state = self.moe_layer_wise_agg(text_hidden_states=t2v_text_hidden_states,
                                                           layer_wise_visual_hidden_states=t2v_layer_wise_visual_hidden_states,
                                                           attention_mask=attention_mask)
        
        v2t_cross_modal_hidden_state = self.moe_layer_wise_agg(text_hidden_states=v2t_vision_hidden_states,
                                                           layer_wise_visual_hidden_states=v2t_layer_wise_text_hidden_states,
                                                           attention_mask=attention_mask)
        
        #plan1
        mixT2vV2t_modal_hidden_state = self.commonExperts(t2v_cross_modal_hidden_state,(v2t_cross_modal_hidden_state,),output_attentions)
        #self.moe_layer_wise_agg(text_hidden_states=v2t_vision_hidden_states,
        #                                                   layer_wise_visual_hidden_states=v2t_layer_wise_text_hidden_states,
        #                                                   attention_mask=attention_mask)
        #commonExpert_output = torch.cat((t2v_cross_modal_attentions,v2t_cross_modal_hidden_state), dim=-1)  # text+img
        

        outputs = (mixT2vV2t_modal_hidden_state[0],)
        if output_attentions:
            mixT2vV2t_cross_modal_attentions = torch.stack(list(mixT2vV2t_modal_hidden_state[1]), dim=1)
            outputs += (t2v_cross_modal_attentions,)
        return outputs
...
class   BertVitInterEncoder(nn.Module):
    def __init__(self, vision_config, text_config, args):
        ...
        self.vision2text_layer_agg = MutualInforLayerWiseAgg(text_config, args, aggred_layer_num=13)
    ...
    def forward(
        ...
        #print(all_text_hidden_states[0].shape,all_vision_hidden_states[0].shape,attention_mask.shape)
        ...
        return BaseModelOutput(
            ...
            hidden_states=(all_text_hidden_states,all_vision_hidden_states),
...
class BertVitInterBaseModel(nn.Module):
    ...
    def forward(
        ...
        vision_embedding_output = self.vision_embeddings(pixel_values, aux_values, rcnn_values)  #bsz * 61 * 768 
        vision_embedding_output = self.vision_pre_layrnorm(vision_embedding_output)
        #print(vision_embedding_output.shape)
